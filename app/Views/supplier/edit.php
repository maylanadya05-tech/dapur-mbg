...
$supplier = $supplier ?? [
  'id' => 1,
  'nama' => 'PT Beras Cianjur Jaya',
  'kategori' => 'Bahan Pokok / Karbohidrat',
  'kontak_nama' => 'H. Endang',
  'no_telp' => '081234567890',
  'email' => 'contact@berascianjur.com',
  'rating' => 5,
  'status' => 'aktif',
  'alamat' => 'Jl. Raya Cianjur No. 102, Cianjur, Jawa Barat'
];
?>
...
<div class="page-header">
  <div class="page-header-left">
    ...
    <p class="page-subtitle">Ubah informasi profil mitra <?= esc($supplier['nama']) ?></p>
  </div>
...
<div style="max-width: 720px;">
  <div class="card" x-data="supplierForm()">
    <div class="card-header" style="margin-bottom:1.5rem; border-bottom:1px solid var(--border-subtle); padding-bottom:1rem; display:flex; justify-content:space-between; align-items:center;">
      <div>
        <h3 class="card-title">Profil Supplier</h3>
        <p class="card-subtitle">Perbarui data kontak atau status kemitraan aktif.</p>
      </div>
      <div>
    ...
    <form action="<?= base_url('/supplier/update/' . $supplier['id']) ?>" method="POST" @submit.prevent="submitForm">
      <?= csrf_field() ?>
      ...
      <div class="form-row" style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;margin-bottom:2rem;">
        <div class="form-group">
          <label class="form-label" style="display:block;margin-bottom:0.5rem;font-weight:600;font-size:0.875rem;color:var(--text-secondary);">Rating Performa</label>
          <select
            name="rating"
            class="form-select"
            x-model="rating"
...
<script>
  function supplierForm() {
    return {
      ...
      kategori: '<?= esc($supplier['kategori']) ?>',
      kontakNama: '<?= esc($supplier['kontak_nama']) ?>',
      noTelp: '<?= esc($supplier['no_telp']) ?>',
      email: '<?= esc($supplier['email']) ?>',
      alamat: '<?= esc($supplier['alamat']) ?>',
      rating: '<?= esc($supplier['rating']) ?>',
