...
$waste = $waste ?? [
  ...
  'qty' => 5.2,
  'satuan' => 'kg',
  'estimasi_kerugian' => 234000,
  'penjelasan' => 'Porsi sisa dari SDN Merdeka 01 yang dikembalikan karena siswa sedang libur kelas lapangan.'
];
...
foreach($ingredientList as $ing) {
  if($ing['id'] == $waste['bahan_id']) {
    $activeIngredient = $ing;
    break;
  }
}
$currentPrice = $activeIngredient ? $activeIngredient['harga_per_unit'] : 0;
?>

<!-- ══ PAGE HEADER ══ -->
<div class="page-header">
  <div class="page-header-left">
    <h1 class="page-title">Edit Laporan Waste</h1>
    <p class="page-subtitle">Ubah laporan limbah bahan makanan terdaftar</p>
  </div>
...
<div style="max-width: 720px;">
  <div class="card" x-data="wasteForm()">
    <div class="card-header" style="margin-bottom:1.5rem; border-bottom:1px solid var(--border-subtle); padding-bottom:1rem;">
      <h3 class="card-title">Form Koreksi Laporan Food Waste</h3>
      <p class="card-subtitle">Ubah kuantitas atau penjelasan sesuai kondisi riil terverifikasi.</p>
    </div>

    <form action="<?= base_url('/sisa/update/' . $waste['id']) ?>" method="POST" @submit.prevent="submitForm">
      <?= csrf_field() ?>
...
<script>
  function wasteForm() {
    return {
      kategori: '<?= esc($waste['kategori']) ?>',
      batchNo: '<?= esc($waste['batch_no'] ?? '') ?>',
      bahanId: '<?= esc($waste['bahan_id']) ?>',
      qty: '<?= esc($waste['qty']) ?>',
      price: <?= $currentPrice ?>,
      satuan: '<?= esc($waste['satuan']) ?>',
      penjelasan: '<?= esc($waste['penjelasan']) ?>',
